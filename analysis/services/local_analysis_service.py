...
def _run_pmd(source_dir: Path) -> list[NormalizedFinding]:
    ...
    return _parse_pmd_findings(data)
...
def _run_spotbugs(source_dir: Path, workspace_dir: Path) -> list[NormalizedFinding]:
    ...
    if proc.returncode not in (0, 1):
        return []
    return _parse_spotbugs_findings(output_path)
...
def _run_semgrep(source_dir: Path) -> list[NormalizedFinding]:
    ...
    data = _load_json_output(proc.stdout)
    return _parse_semgrep_findings(data)
...
def _collect_custom_metrics(source_dir: Path) -> tuple[list[NormalizedFinding], dict[str, float]]:
    ...
    for file_path in java_files:
        ...
        file_findings, ccn_delta, nloc_delta = _collect_lizard_findings(analysis, rel)
        findings.extend(file_findings)
        total_ccn += ccn_delta
        total_nloc += nloc_delta
        _collect_javalang_findings(
            file_path=file_path,
            rel_path=rel,
            findings=findings,
...
def _build_metrics(findings: list[NormalizedFinding], lizard_summary: dict[str, float]) -> LocalAnalysisMetrics:
    ...
    maintainability_rating = _rating_from_thresholds(code_smells, (10, 30, 60), (1, 2, 3, 4))
    reliability_rating = _rating_from_thresholds(bugs, (1, 3), (1, 2, 3))
    security_rating = _rating_from_thresholds(vulnerabilities, (1, 3), (1, 2, 3))
    quality_gate_status = _resolve_quality_gate_status(
...
def _load_json_output(raw_stdout: str | None) -> dict:
...
def _parse_pmd_findings(data: dict) -> list[NormalizedFinding]:
    findings: list[NormalizedFinding] = []
    for item in data.get("files", []) or []:
        file_path = str(item.get("filename") or "")
        for violation in item.get("violations", []) or []:
            findings.append(_pmd_violation_to_finding(file_path, violation))
    return findings


def _pmd_violation_to_finding(file_path: str, violation: dict) -> NormalizedFinding:
...
def _parse_spotbugs_findings(output_path: Path) -> list[NormalizedFinding]:
    ...
    try:
        ...
        return []
    root = tree.getroot()
    return [_spotbugs_bug_to_finding(bug) for bug in root.findall(".//BugInstance")]


def _spotbugs_bug_to_finding(bug) -> NormalizedFinding:
...
def _parse_semgrep_findings(data: dict) -> list[NormalizedFinding]:
    findings: list[NormalizedFinding] = []
    for item in data.get("results", []) or []:
        findings.append(_semgrep_item_to_finding(item))
    return findings


def _semgrep_item_to_finding(item: dict) -> NormalizedFinding:
...
def _collect_lizard_findings(analysis, rel_path: str) -> tuple[list[NormalizedFinding], int, int]:
    findings: list[NormalizedFinding] = []
    total_ccn = 0
    total_nloc = int(analysis.nloc or 0)
    for fn in analysis.function_list:
        ccn = int(fn.cyclomatic_complexity or 0)
        total_ccn += ccn
        if ccn >= 15:
            findings.append(
                _metric_finding(
                    "lizard",
                    "HIGH",
                    "metric:high_cyclomatic_complexity",
                    rel_path,
                    fn.start_line,
                    f"Método con complejidad ciclomática alta: {fn.name} ({ccn}).",
                )
            )
        length = int(fn.length or 0)
        if length >= 60:
            findings.append(
                _metric_finding(
                    "lizard",
                    ...
                    rel_path,
                    fn.start_line,
                    f"Método muy largo: {fn.name} ({length} líneas).",
                )
            )
        params = int(fn.parameter_count or 0)
        if params >= 6:
            findings.append(
                _metric_finding(
                    "lizard",
                    "MEDIUM",
                    "metric:too_many_parameters",
                    rel_path,
                    fn.start_line,
                    f"Método con demasiados parámetros: {fn.name} ({params}).",
                )
            )
    return findings, total_ccn, total_nloc


def _collect_javalang_findings(
    *,
    file_path: Path,
    rel_path: str,
    findings: list[NormalizedFinding],
    ...
    for _, node in parsed:
        ...
        findings.append(
            _metric_finding(
                ...
                "metric:deep_nesting",
                rel_path,
                getattr(getattr(node, "position", None), "line", None),
                f"Profundidad de anidación alta ({nesting}) en método {node.name}.",
            )
        )


def _rating_from_thresholds(value: int, thresholds: tuple[int, ...], ratings: tuple[int, ...]) -> int:
...
def _resolve_quality_gate_status(*, vulnerabilities: int, bugs: int, code_smells: int, complexity: int) -> str:
    if vulnerabilities > 0 or bugs > 0:
        return "FAILED"
    if code_smells > 20 or complexity > 300:
        return "WARN"
    return "OK"
